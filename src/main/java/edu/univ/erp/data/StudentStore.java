...
public class StudentStore {
    ...
    /** Fetch student record from ERP DB only (no Auth DB join) */
    public Student findById(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getErpConnection();
            ...
            if (rs.next()) {
                ...
                student.setProgram(rs.getString("program"));
                student.setYear(rs.getInt("year"));
                // Username will be fetched separately from Auth DB in the service layer
                return student;
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    /** Insert new student record (ERP DB) */
    public boolean create(Student student) {
        String sql = "INSERT INTO students (user_id, roll_no, program, year) VALUES (?, ?, ?, ?)";
    ...
    /** Fetch all students from ERP DB */
    public List<Student> findAll() {
        List<Student> students = new ArrayList<>();
        String sql = "SELECT user_id, roll_no, program, year FROM students";
